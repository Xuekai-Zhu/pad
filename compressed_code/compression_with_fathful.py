...
# "compressed_code/results_compressed_ratio.csv"
fathiful_data = "metric/results_for_enhanced_data/fathiful.tsv"
df_fathiful = pd.read_csv(fathiful_data)
...
merge_df = pd.concat([df_fathiful, df_data], axis=1)
merge_df = merge_df.reset_index()
key = df_fathiful.columns.tolist()
sorted_df = merge_df.sort_values(by=key[0])
print(sorted_df)
# sorted_df.to_csv("dataset/gsm8k-generat-data/enhanced_data/filter_data/compress_retio/sorted_data.csv")

save_dir = "dataset/gsm8k-generat-data/enhanced_data/filter_data/faithful_filter/"
...
def save_to_json(ratio, sorted_df, save_dir):
    ...
    top_n = sorted_df["data"].iloc[-n:].to_list()
