...
def main():
    ...
    def preprocess_function(example):
        ...
        inputs = [d + " "+ i for d, i in zip(example["definition"], example["inputs"])]
        ...
        # padding in the loss.
        if args.ignore_pad_token_for_loss == True:
                labels["input_ids"] = [
   ...
   # Split weights in two groups, one with weight decay and the other not.
    ...
    optimizer = GeminiAdamOptimizer(optimizer_grouped_parameters, lr=args.learning_rate, initial_scale=2**5, gpu_margin_mem_ratio=0.0)
