    ...
    def update_evaluator(self, in_file: str):
        ...
        with open(in_file) as _f:
            for line in _f:
                ...
                h_chain = ReasoningSteps(line=jline["code"], type="code")
                # context = ReasoningSteps(
                #     line=jline["premise"] + " " + jline["hypothesis"]
                # )
                context = ReasoningSteps(jline["question"])
                # if "gsm8k" in in_file:
...
if __name__ == '__main__':
    ...
    # score_list = ['faithfulness', 'faithfulness_ww', 'repetition_word', 'repetition_step', 'informativeness_step', 'informativeness_chain', 'discourse_representation', 'coherence_step_vs_step', 'perplexity_step', 'perplexity_chain', 'grammar_step', 'perplexity_step_max', 'grammar_step_max', 'reasoning_alignment', 'external_hallucination', 'redundancy', 'common_sense_error', 'missing_step', 'semantic_coverage_step', 'semantic_coverage_chain']
    
    
    # input command
    # 4 after then use refine-train-delete-question.json
    opt["model_type"] = "sim_sce"
    opt['model_name'] = "facebook/roscoe-512-roberta-base"
    opt["dataset_path"] = "../dataset/gsm8k-generat-data/enhanced_data/7_time_data/train-enhanced.json"
    # opt["dataset_path"] = "../dataset/gsm8k-generat-data/gsm8k-8/refine-train.json"
    opt['scores'] = [
                    'faithfulness', 
                    #  'repetition_step', 
    ...
    #     print(f"Score file for {opt['dataset_path']} already exists. Skipping.")
    # else:
    print(f"Evaluating {opt['dataset_path']}")
    evaluator.update_evaluator(opt['dataset_path'])
    score_types = (
        REASONING_SCORES
        if "esnli" in opt['dataset_path'] or "gsm8k" in opt['dataset_path']
