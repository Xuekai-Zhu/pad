...
from data_preprocess import GSM8K_DataFunction, GSM8K_Add_Solution_DataFunction
...
parser.add_argument("--max_new_tokens", default=256, type=int, help="maximum new tokens to generate")
...
parser.add_argument("--output_dir", required=False, default=None, type=str, help="save-path to store the output")
# parser.add_argument("--add_solution", action="store_true", help="add solution to input before question")
...
#     eval_dataset = GSM8K_Add_Solution_DataFunction(args.local_dataset_path)
#     eval_dataset = GSM8K_DataFunction(args.local_dataset_path)
#     print("use  gsm8k add solution")
# else:
eval_dataset = GSM8K_DataFunction(args.local_dataset_path)
...
for step, batch in enumerate(tqdm(eval_dataloader)):
    ...
    start = time.time()
    outputs = pipe(batch[column_name],
            num_tokens=args.max_new_tokens,
            do_sample=(not args.greedy))
    # delete the input in the output
    outputs = [i.replace(j, "") for i, j in zip(outputs, batch[column_name])]
    
    all_output_text += outputs
    torch.cuda.synchronize()
    end = time.time()
...
if args.output_dir is not None:
    ...
    save_file = os.path.join(args.output_dir, "results.txt")
    with open(save_file, "w") as f:
        for i in tqdm(all_output_text):
            item = {"output": i}
