...
class DSPipeline():
    ...
    def generate_outputs(self,
        ...
        input_tokens = self.tokenizer(inputs, return_tensors="pt", padding=True, max_length=max_input_length)
        ...
        # batch_decode cant remove the special token within batch, so i reviesd the batch decode to single sample decode.
        # outputs_list = []
        # for t in range(outputs.size[0]): 
        #     outputs_t = self.tokenizer.batch_decode(outputs[t], skip_special_tokens=True, clean_up_tokenization_spaces=False)
